...
def gradient_descent(x_init, learning_rate, iteration):
    ...
    for i in range(iteration):
        ...
        # if (np.linalg.norm(grad(x_new))/len(x_new) < .5):
        #     break #when to break
...
#random intial line 
x_init = np.array([[1.], [2.]]) #cai nay la intercept voi coef
...
def update(i):
    y0 = x_list[i][0][0] + x_list[i][1][0]*x0_gd
    line.set_data(x0_gd, y0)
    return line,
...
#legend for plot
plt.legend(('Value in each GD iteration', 'Solution by formular', 'Initial value for GD'), loc=(.52, .01))
...
for i in range(len(x_list)):
    ...
    plt.plot(x0_gd, y0_x_list, color="black", alpha = .3)
plt.show()


# #draw plot for cost per iteration to determine when to stop
# cost_list = []
# iter_list = []
# for i in range(len(x_list)):
#     iter_list.append(i)
